...
import { ArrowRight, Image as ImageIcon, Paintbrush } from "lucide-react";
import { getHomepageData } from "@/lib/server/pages";

const STRAPI_URL = process.env.NEXT_PUBLIC_STRAPI_URL || '';

export default async function DesignServices() {
  const homepageData = await getHomepageData();
  
  // Get services from homepage relation data
  const designServices = homepageData?.designServices || [];

  // Get section content from CMS
  const tagline = homepageData?.designServicesTagline || "Dịch vụ thiết kế";
  const title = homepageData?.designServicesTitle || "THIẾT KẾ NỘI THẤT";
  const description = homepageData?.designServicesDescription || 
    "Dịch vụ thiết kế nội thất chuyên nghiệp cho mọi không gian sống";
  const buttonText = homepageData?.designServicesButtonText || "Xem tất cả";
  const buttonLink = homepageData?.designServicesButtonLink || "/thiet-ke";
